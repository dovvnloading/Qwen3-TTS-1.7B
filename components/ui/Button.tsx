...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variants = {
    primary:
      'bg-background text-strong rounded-lg shadow-neu-flat hover:text-strong/90 active:shadow-neu-pressed',
    secondary:
      'bg-background text-text rounded-lg shadow-neu-flat hover:text-strong active:shadow-neu-pressed',
    icon: 'bg-background text-text-muted rounded-full shadow-neu-icon hover:text-strong active:shadow-neu-icon-pressed',
    'active-icon': 'bg-background text-strong rounded-full shadow-neu-icon-pressed',
  };
    ...
    >
      {loading && (
        <span className={`absolute inset-0 flex items-center justify-center bg-background ${radiusClass} z-10`}>
          <svg className="animate-spin h-4 w-4 text-strong/80" viewBox="0 0 24 24">
